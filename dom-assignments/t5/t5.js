...
// Lisätään OpenStreetMap-laatta
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
  attribution: '&copy; <a href="https://www.openstreetmap.org/">OpenStreetMap</a> tekijät'
}).addTo(map);

// Käydään läpi ravintolat ja lisätään markkerit
restaurants.forEach(ravintola => {
  const [lon, lat] = ravintola.location.coordinates;

  const marker = L.marker([lat, lon]).addTo(map);

  // Ponnahdusikkunan sisältö
  const popupContent = `
    <h3>${ravintola.name}</h3>
    <p>${ravintola.address}, ${ravintola.postalCode} ${ravintola.city}</p>
  `;
